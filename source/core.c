...
const char* arb64_tty = "";
...
void compile_and_update() {
    ...
    if (chdir("source") != 0) {
        core_print(RED " ✖ [ERROR] 'source' folder not found for compilation!\n" RESET);
        return;
    }

    loading_animation("Compiling Server Module");
    system("gcc server.c api.c cJSON.c -o arc_server -lssl -lcrypto -pthread -w -g");

    loading_animation("Updating Core Orchestrator");
    system("gcc core.c -o core -pthread -w");

    core_print(GRAY "\n ⮑  Moving binaries and configuring permissions...\n" RESET);
...
void cleanup(int sig) {
    core_print("\n\n" RED " ⚠ " WHITE "Interrupt received. Terminating ARCAPI...\n" RESET);
...
void program() {
    ...
    core_print(GREEN " ╔══════════════════════════════════════════════╗\n" RESET);
    core_print(GREEN " ║  " WHITE "ARCAPI Core Status: " GREEN "ONLINE                  ║\n" RESET);
    core_print(GREEN " ╚══════════════════════════════════════════════╝\n\n" RESET);
    ...
    if (global_server_pid == -1) {
        ...
        while (1) {
            if (pending_action == 1) {
                pending_action = 0;
                core_print(YELLOW "\n ⟳ [CORE] Hot-Reloading API Module...\n" RESET);
                const char *cmd[] = {"gcc", "server.c", "api.c", "cJSON.c", "-o", "../arc_server", "-lssl", "-lcrypto", "-pthread", "-w", "-g", NULL};
                safe_compile(cmd, "source");
                ...
                pending_action = 0;
                core_print(YELLOW "\n ⟳ [CORE] Performing Full Core Restart...\n" RESET);
                const char *cmd_srv[] = {"gcc", "server.c", "api.c", "cJSON.c", "-o", "../arc_server", "-lssl", "-lcrypto", "-pthread", "-w", "-g", NULL};
                safe_compile(cmd_srv, "source");
                const char *cmd_core[] = {"gcc", "core.c", "-o", "../core", "-pthread", "-w", NULL};
                safe_compile(cmd_core, "source");
                if (global_server_pid > 0) { kill(global_server_pid, SIGTERM); waitpid(global_server_pid, NULL, 0); }
                char *args[] = { "./core", NULL };
                execv(args[0], args); exit(1);
            }
